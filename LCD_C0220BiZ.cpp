...
const uint8_t LINE_BASE_ADDR = 0x40; // Line 2 base address in DDRAM
...
void LCD_C0220BIZ::setDelay (int cmdDelay,int charDelay) 
{
	_cmdDelay = cmdDelay;
...
void LCD_C0220BIZ::clear()
{
   command (CLEAR_DISP_CMD);
}


void LCD_C0220BIZ::home()
{
   command ( HOME_CMD );
}


void LCD_C0220BIZ::on()
{   
   command ( DISP_ON_CMD );
}


void LCD_C0220BIZ::off()
{
   command ( DISP_OFF_CMD );        
}


void LCD_C0220BIZ::cursor_on()
{
   command ( DISP_ON_CMD | CURSOR_ON_BIT );
}

void LCD_C0220BIZ::cursor_off()
{
   command ( DISP_ON_CMD & ~(CURSOR_ON_BIT) );
}

void LCD_C0220BIZ::blink_on()
{
   command ( DISP_ON_CMD | BLINK_ON_BIT );
}

void LCD_C0220BIZ::blink_off()
{
   command ( DISP_ON_CMD & ~(BLINK_ON_BIT) ); 
}
...
#ifdef _LCDEXPANDED
uint8_t LCD_C0220BIZ::status(){
	
	return _status;
}


uint8_t LCD_C0220BIZ::keypad ()
{
   // NOT SUPPORTED
   return 0;
}
...
void LCD_C0220BIZ::setBacklight(uint8_t new_val)
{
...
void LCD_C0220BIZ::setContrast(uint8_t new_val)
{
